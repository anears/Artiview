...
에이전트(Claude Code, Codex, 사내 파이프라인…)가 생성한 **HTML 결과물·발표자료와
Markdown 문서**를 한곳에 모아 보고 관리하는 macOS 데스크톱 앱입니다. 브라우저로는
번거로운 **라이브러리 관리 · 전문 검색 · 라이브 렌더 썸네일 · 즐겨찾기·태그**를
제공하고, 로컬 폴더뿐 아니라 **SSH 서버의 원격 폴더**도 등록할 수 있습니다.
로컬 폴더는 자동으로 감시되어 새 파일이 알아서 나타납니다.

![Artiview 데모 — 라이브러리 탐색, 전문 검색, 문서 내 검색, 라이트/다크 테마](docs/demo.gif)

- **원본 파일을 절대 수정·삭제하지 않습니다.** 색인만 관리하며, "라이브러리에서
  제거"도 색인 항목만 지웁니다.
- **로컬 우선.** 모든 데이터는 로컬 SQLite에 저장되고, SSH 비밀번호는 디스크에
  기록되지 않습니다.
- **신뢰할 수 없는 HTML은 샌드박스에서.** 뷰어·썸네일 iframe은
  `allow-same-origin` 없이 격리되어 악성 문서가 앱 권한에 접근할 수 없습니다.
...
UI는 기본 영어이며, 시스템 언어가 한국어면 자동으로 한국어로 표시됩니다.
라이트/다크 테마는 시스템 설정을 따르고, 언어와 테마 모두 사이드바 하단의
**⚙ 설정**에서 직접 지정할 수 있습니다.

## 시작하기

앱을 처음 열면 라이브러리가 비어 있습니다. 세 가지 방법으로 문서를 추가하세요.
...
등록한 **로컬 폴더는 자동으로 감시**됩니다 — 에이전트가 새 파일을 만들거나
기존 파일을 다시 쓰면 몇 초 안에 라이브러리에 반영됩니다. 원격 폴더는
**새로고침 ↻** 버튼으로 변경분만 다시 스캔합니다.
...
- Windows/Linux 빌드는 최근 추가되어 테스트가 충분하지 않습니다 — 이슈 제보 환영합니다.
  macOS는 Apple Silicon 빌드만 제공됩니다.
